...
const bgmEnabled = ref(true) // BGM 独立开关
const masterVolume = ref(0.8) // 主音量 0-1
const bgmVolume = ref(0.15) // BGM 音量 0-1，置底的关键：音量低
const sfxVolume = ref(0.6) // 音效音量 0-1

// ===== 音频对象缓存 =====
const sfxCache = new Map()
let currentBgm = null
let bgmPrompted = false // 记录是否已经弹窗询问过用户

// ===== 预加载音效 =====
const SFX_LIST = [
  'click', 'win', 'lose', 'step', 'hit', 'pickup', 'gameover'
]

function getSfxPath(name) {
  return `/assets/sounds/sfx/${name}.mp3`
}

function getBgmPath(name) {
  return `/assets/sounds/bgm/${name}.mp3`
}
...
function updateAllVolumes() {
  ...
  if (currentBgm) {
    currentBgm.volume = bgmVolume.value * masterVolume.value
  }
}
...
export function stopBgm(fadeOut = 1000) {
  ...
  if (fadeOut > 0) {
    fadeAudio(bgm, bgm.volume, 0, fadeOut, () => {
      bgm.pause()
      bgm.currentTime = 0
    })
  } else {
...
export function useSound() {
  ...
  return {
    soundEnabled, bgmEnabled, masterVolume, bgmVolume, sfxVolume,
    initSounds,
    playBgm, stopBgm, switchBgm,
    playSfx,
