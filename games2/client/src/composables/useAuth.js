/**
 * 认证与用户资产组合函数 V3（全局单例版 + 晶石支持）
 * [V3] 新增 crystal/displayCrystal 状态管理，底层放大1000倍防浮点精度丢失
 * [V2] 新增 uid/nickname/avatar 状态管理 + 修改昵称/头像方法
 * [Fix] 新增 stopBalanceAnimation 清理定时器，防止组件卸载后报错
 */
...
function animateCrystal(target) {
  // 底层按1000倍整数运算，防止0.1+0.2的浮点精度问题
  const start = Math.round(displayCrystal.value * 1000)
  const end = Math.round(target * 1000)
  ...
  crystalTimer = setInterval(() => {
    step++
    // 运算时用整数，显示时除以1000还原
    displayCrystal.value = Math.round(start + stepVal * step) / 1000
    if (step >= steps) {
