/**
 * 认证与用户资产组合函数 V4（全局单例版 + 晶石支持 + 维护模式拦截）
 * [V4] 修复晶石状态不更新问题，统一余额和晶石的动画与存储逻辑
 * [V5] 新增：503 维护模式全局拦截，普通用户进不来
 */
  ...
  try {
    ...
    if (res.status === 503) {
      ...
      if (data.code === 'MAINTENANCE') {
        logout(); // 1. 清理本地 Token 和用户状态
        alert('系统升级维护中，请稍后再试'); // 2. 提示用户
        // 3. 强制跳转到登录页 (根据你的前端路由模式，如果是 hash 模式用 '/#/login')
        window.location.href = '/login'; 
        throw new Error('系统维护中'); // 4. 阻断后续逻辑
      }
    }
...
async function fetchBalance() {
  try {
    ...
    if (res.ok) { updateBalance(data.balance ?? data?.data?.balance) }
  } catch (e) { console.warn('[全局余额] /api/balance 获取失败', e.message) }
}
...
async function login(phone, password) {
  ...
  if (data.user?.balance !== undefined) { updateBalance(data.user.balance) }
  else { await fetchBalance() }
  if (data.user?.crystal !== undefined) { updateCrystal(data.user.crystal) }
  return data
}
...
function restoreUser() {
  ...
  if (token.value && phone) {
    currentUser.value = { phone, role: role || 'user', uid: uid || '', nickname: nickname || '', avatar: avatar ? Number(avatar) : 1 }
  }
}
...
export function useAuth() {
  return {
    ...
    // 余额相关
    balance, displayBalance, balanceAnimating, fetchBalance, updateBalance, stopBalanceAnimation,
    // ★ 晶石相关
    crystal, displayCrystal, crystalAnimating, updateCrystal, stopCrystalAnimation,
    // 历史 & 明细
