/** * 客服系统 - 组合函数 v3 * [v3] 纯推送架构：消息存localStorage，会话列表WebSocket推送 * - 不再轮询 /api/chat/sessions * - 不再拉取 /api/chat/history * - 消息持久化在浏览器 localStorage */ 
import { ref } from 'vue' 
import { useAuth } from './useAuth.js' 
import { addVersion } from '../config' // 引入版本号工具
...
function saveCurrentSession(sessionId) { 
  try { 
    localStorage.setItem(STORAGE_CURRENT_SESSION, sessionId || '') 
  } catch {} 
} 

function loadCurrentSession() { 
  try { 
    return localStorage.getItem(STORAGE_CURRENT_SESSION) || null 
  } catch { 
    return null 
  } 
} 
...
function saveSessionListToLocal(list) { 
  try { 
    localStorage.setItem(STORAGE_SESSION_LIST, JSON.stringify(list)) 
  } catch {} 
} 
...
function playNotificationSound(sender) { 
  try { 
    ...
    if (!notificationAudio) { 
      notificationAudio = new Audio(addVersion('/assets/sounds/sfx/click.mp3')) 
...
export function useChat() { 
      ...
      switch (data.type) { 
        ...
        case 'chat_message_sent': 
          if (data.message) { 
            ...
            if (data.type === 'chat_message') { 
              ...
              if ((role === 'admin' && sender === 'user') || (role !== 'admin' && (sender === 'admin' || sender === 'bot'))) { 
                unreadCount.value += 1 
        ...
        case 'session_update': 
          if (role === 'admin' && data.session) { 
            ...
            if (idx !== -1) { 
              sessionList.value.splice(idx, 1, data.session) 
            } else { 
              sessionList.value.unshift(data.session) 
            } 
            saveSessionListToLocal(sessionList.value) 
          } 
          break 
  ...
  return { 
    ...
    sessionStatus, 
    adminName, 
    currentSessionId, // ★ 必须导出，供 AdminChat 使用
    sessionList, 
    connect, 
    disconnect, 
    sendMessage, 
