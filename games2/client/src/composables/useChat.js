...
export function useChat() {
    ...
    wsInstance.onopen = async () => {
      ...
      if (!currentSessionId.value) {
        ...
        if (activeSession) {
          ...
          sessionStatus.value = activeSession.status
        }
      }

      // 只有存在 sessionId 时才加载历史，否则后端会报 400
      if (currentSessionId.value) {
      ...
      switch (data.type) {
        ...
        case 'chat_message_sent':
          if (data.message) {
            ...
            if (!exists) {
              if (role === 'admin' && currentSessionId.value && data.message.sessionId !== currentSessionId.value) {
  ...
  async function fetchSessions() {
    ...
    try {
      ...
      if (res.ok && data.sessions) {
        return data.sessions.map(s => ({ ...s, id: s.id || String(s._id) }))
      }
      return []
    } catch (err) {
  ...
  return {
    ...
    acceptSession, closeSession,
    fetchSessions, resetSession, clearUnread
  }
}
