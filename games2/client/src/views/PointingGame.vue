...
const characterBets = reactive({ '赵云': 0, '关羽': 0, '张飞': 0, '马超': 0, '秦良玉': 0, '梁红玉': 0, '穆桂英': 0, '花木兰': 0 })
...
function revokeBet(type, name) {
  ...
  if (amount > 0) {
    // 退还余额
    updateBalance(balance.value + amount)
  }
}
...
async function startGame() {
  ...
  // 此时 balance.value 已经是扣除 totalBetAmount 后的了
  ...
  try {
    ...
    // 🔥 结算成功：以后端返回的权威余额为准，直接覆盖本地余额
    // 后端逻辑依然是：数据库余额 - 本金 + 奖金 = 最终余额返回
...
function buildResultDetails(data, bets) {
  const details = bets.map(bet => {
    ...
    if (data.result.resultType === 'all_survived') {
      isWin = true; payout = bet.type === 'gender' ? bet.amount * 1.9 : bet.amount * characters.value.find(c=>c.name===bet.choice).odds
      ...
      if (bet.type === 'gender') {
        ...
        if (data.result.survivedCharacters.includes(bet.choice)) { isWin = true; payout = bet.amount * characters.value.find(c=>c.name===bet.choice).odds }
      }
    }
    return { choice: bet.choice, amount: bet.amount, isWin, payout: Math.floor(payout) }
  })
  return {
    totalWin: data.netChange > 0, netChange: data.netChange, details,
...
async function playAssassinationAnimation(backendResult) {
  ...
  let currentIndex = 0; let speed = 80
  const emptyRounds = 8 + Math.floor(Math.random() * 8)
  for (let i = 0; i < emptyRounds; i++) {
    highlightIndex.value = currentIndex % 8; currentIndex++; await sleep(speed)
  }
  ...
  for (let i = 0; i < toEliminate.length; i++) {
    ...
    for (let s = 0; s < stepsToTarget; s++) {
      highlightIndex.value = currentIndex % 8; currentIndex++; speed = 80 + (s * 40); await sleep(speed)
    }
    animatingEliminated.value.push(targetName)
    screenShaking.value = true; setTimeout(() => screenShaking.value = false, 300); await sleep(1200); speed = 80
  }

  if (backendResult.resultType === 'all_survived') { highlightIndex.value = -1; await sleep(500) }
  highlightIndex.value = -1
}

function closeResult() {
  showResult.value = false
  // 关闭结算时不需要退钱，因为 startGame 里已经重置过了
}
...
.game-app {
  ...
  background-size: 140% auto; background-position: center; background-attachment: fixed;
