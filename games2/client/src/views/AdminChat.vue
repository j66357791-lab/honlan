        ...
        <div class="drawer-panel">
          <div class="drawer-tabs">
            <button :class="{ active: listTab === 'waiting' }" @click="listTab = 'waiting'">
              等待 <span v-if="waitingSessions.length" class="tab-count">{{ waitingSessions.length }}</span>
            </button>
            <button :class="{ active: listTab === 'active' }" @click="listTab = 'active'">
              进行中 <span v-if="activeSessions.length" class="tab-count">{{ activeSessions.length }}</span>
            </button>
            <button :class="{ active: listTab === 'closed' }" @click="listTab = 'closed'">已结束</button>
          </div>
          <div class="drawer-list">
            <div v-for="s in currentListSessions" :key="s.id" class="s-card"
    ...
    <div v-else class="chat-main">
      ...
      <Transition name="slide">
        <div v-if="showAdjust && currentSession?.status === 'active'" class="adjust-panel">
          <div class="ap-row">
            <select v-model="adjustType"><option value="add">➕上分</option><option value="sub">➖下分</option></select>
      ...
      <div class="msg-area" ref="msgEl">
        <div v-for="msg in messages" :key="msg.id" class="m-row"
          :class="{ 'm-self': msg.sender === 'admin', 'm-other': msg.sender === 'user' || msg.sender === 'bot', 'm-sys': msg.sender === 'system' }">
          <div v-if="msg.sender === 'system'" class="m-sys-text">{{ msg.content }}</div>
          <template v-else>
            <div class="m-av" :class="msg.sender">{{ msg.sender === 'bot' ? '🤖' : msg.sender === 'admin' ? '🧑‍💼' : '👤' }}</div>
            <div class="m-body">
...
// ★ 用 sessionList 替代本地 sessions，用 switchSession 替代 loadHistory
const { connected, messages, connect, sendMessage, markRead, acceptSession: wsAccept, closeSession: wsClose, sessionList, switchSession } = useChat()
...
// ★ 直接用 sessionList（WebSocket推送），不再本地维护
const waitingSessions = computed(() => sessionList.value.filter(s => s.status === 'waiting'))
const activeSessions = computed(() => sessionList.value.filter(s => s.status === 'active'))
const closedSessions = computed(() => sessionList.value.filter(s => s.status === 'closed'))
...
function selectSession(s) {
  ...
  switchSession(sid) // ★ 从localStorage加载消息 + 标记已读
...
// ★ 当 sessionList 更新时，同步 currentSession 的最新状态
...
onMounted(() => {
  connect()
  // ★ 不再需要 setInterval 轮询！会话列表由 WebSocket 推送
})
</script>

<style scoped>
.admin-chat { height: 100vh; background: #0f1923; color: #e0e0e0; display: flex; flex-direction: column; overflow: hidden; }
.top-bar { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: #162231; border-bottom: 1px solid rgba(255,255,255,0.05); flex-shrink: 0; z-index: 10; }
.tb-btn { position: relative; width: 42px; height: 42px; border-radius: 10px; border: none; background: rgba(255,255,255,0.06); color: #ccc; font-size: 20px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
.tb-btn:active { background: rgba(255,255,255,0.12); }
.tb-badge { position: absolute; top: -4px; right: -4px; background: #e74c3c; color: #fff; font-size: 10px; min-width: 16px; height: 16px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 700; }
.tb-center { font-size: 15px; font-weight: 600; color: #f0f0f0; display: flex; align-items: center; gap: 6px; }
.tb-balance { font-size: 12px; color: #f0c040; }
.drawer-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 50; display: flex; }
.drawer-panel { width: 85%; max-width: 360px; background: #131d2a; display: flex; flex-direction: column; border-right: 1px solid rgba(255,255,255,0.05); overflow: hidden; }
.drawer-tabs { display: flex; border-bottom: 1px solid rgba(255,255,255,0.05); flex-shrink: 0; }
.drawer-tabs button { flex: 1; padding: 12px 0; border: none; background: transparent; color: #666; font-size: 13px; cursor: pointer; position: relative; transition: color 0.2s; }
.drawer-tabs button.active { color: #f0c040; }
.drawer-tabs button.active::after { content: ''; position: absolute; bottom: 0; left: 20%; right: 20%; height: 2px; background: #f0c040; border-radius: 1px; }
.tab-count { background: rgba(240,192,64,0.15); color: #f0c040; font-size: 10px; padding: 1px 5px; border-radius: 6px; margin-left: 3px; }
.drawer-list { flex: 1; overflow-y: auto; padding: 8px; }
.s-card { display: flex; justify-content: space-between; align-items: center; padding: 12px; border-radius: 10px; cursor: pointer; transition: background 0.12s; margin-bottom: 2px; }
.s-card:active { background: rgba(255,255,255,0.04); }
.s-card.active { background: rgba(240,192,64,0.1); }
...
.empty-state { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; color: #444; }
.es-icon { font-size: 52px; }
.es-text { font-size: 14px; }
.es-hint { font-size: 13px; color: #f39c12; font-weight: 600; }
.chat-main { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
.user-strip { display: flex; justify-content: space-between; align-items: center; padding: 8px 14px; background: #162231; border-bottom: 1px solid rgba(255,255,255,0.04); flex-shrink: 0; }
...
.adjust-panel { padding: 12px 14px; background: #1a2836; border-bottom: 1px solid rgba(255,255,255,0.04); flex-shrink: 0; }
...
.input-bar { display: flex; gap: 8px; padding: 10px 14px; padding-bottom: calc(10px + env(safe-area-inset-bottom, 0px)); background: #162231; border-top: 1px solid rgba(255,255,255,0.05); flex-shrink: 0; }
