      ...
      <div v-if="showList" class="drawer-overlay" @click.self="showList = false">
        <div class="drawer-panel">
          ...
          <div class="drawer-list">
            <div
              v-for="s in currentListSessions" :key="s.id"
              class="s-card" :class="{ active: currentSessionId === s.id }"
    ...
    <div v-else class="chat-main">
      ...
      <div class="msg-area" ref="msgEl">
        <div
          ...
          class="m-row"
          :class="{
            'm-self': msg.sender === 'admin',
            'm-other': msg.sender === 'user' || msg.sender === 'bot',
            'm-sys': msg.sender === 'system'
          }"
        >
...
const {
  connected, messages, connect, sendMessage, markRead, loadHistory,
  acceptSession: wsAccept, closeSession: wsClose, fetchSessions
} = useChat()
...
.drawer-tabs button.active::after {
  ...
  height: 2px; background: #f0c040; border-radius: 1px;
}
.tab-count {
