  ...
  <div class="admin-panel">
    ...
    <div class="card-grid">
      ...
      <div class="admin-card">
        ...
        <h3>风控大盘</h3>
        <p>系统盈亏、风控特征分布、内部账号管理</p>
        <div class="card-actions">
          <button @click="showRiskDashboard = true">查看大盘</button>
        </div>
      </div>
      ...
      <div class="admin-card danger-card">
        <div class="card-icon">☢️</div>
        <h3>系统维护</h3>
        <p>开发测试专用：一键清理对局、清空盈亏、重置数据</p>
        <div class="card-actions">
          <button class="danger-btn" @click="showMaintenance = true">进入维护</button>
        </div>
      </div>
    ...
    <div v-if="showRiskDashboard" class="modal-overlay" @click.self="showRiskDashboard = false">
      <div class="modal-content large-modal">
        <div class="modal-header">
          <h2>🛡️ 风控大盘数据</h2>
          <button class="close-btn" @click="showRiskDashboard = false">✕</button>
        </div>
        <div class="modal-body">
          <div class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <div class="stat-value text-red">{{ users.filter(u => u.isHighRisk).length }}</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">⚪ 白名单用户</div>
              <div class="stat-value text-blue">{{ users.filter(u => u.isWhitelisted).length }}</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">🛡️ 内部测试号</div>
              <div class="stat-value text-gold">{{ users.filter(u => u.isInternal).length }}</div>
            </div>
            <div class="stat-card">
        ...
        <div class="modal-body">
          ...
          <div class="user-list">
            <div v-for="user in filteredUsers" :key="user._id" class="user-row">
              <div class="user-info">
                <img v-if="user.avatar" :src="AVATAR_MAP[user.avatar] || DEFAULT_AVATAR" class="user-avatar-small" />
                <span v-else class="user-av-placeholder">👤</span>
                ...
                <span class="user-internal" v-if="user.isInternal">🛡️内部</span>
              ...
              <div class="user-actions">
                ...
                <button class="sm-btn profile-btn" @click="openEditProfile(user)">修改资料</button>
                <button class="sm-btn internal-btn" @click="toggleInternal(user)">
                  {{ user.isInternal ? '取消内部' : '设为内部' }}
                </button>
                <button class="sm-btn risk-btn" @click="toggleRisk(user)">
                  {{ user.isHighRisk ? '取消高危' : '标记高危' }}
                </button>
                <button class="sm-btn white-btn" @click="toggleWhitelist(user)">
                  {{ user.isWhitelisted ? '取消白名单' : '加白名单' }}
                </button>
                <button class="sm-btn" :class="user.banned ? 'unban-btn' : 'ban-btn'" @click="toggleBan(user)">
                  {{ user.banned ? '解封' : '封禁' }}
                </button>
              </div>
            </div>
    ...
    <div v-if="showEditProfile" class="modal-overlay" @click.self="showEditProfile = false">
      <div class="modal-content small-modal">
        <div class="modal-header">
          <h2>修改用户资料 - {{ editProfileTarget?.phone }}</h2>
        ...
        <div class="modal-body">
          <div class="edit-profile-form">
            ...
            <div class="form-actions" style="margin-top: 16px;">
              ...
              <button class="confirm-btn" @click="submitEditProfile" :disabled="!canSubmitProfile || submittingProfile">
                {{ submittingProfile ? '提交中...' : '确认修改' }}
              </button>
            </div>
          </div>
        </div>
    ...
    <div v-if="showMaintenance" class="modal-overlay" @click.self="showMaintenance = false">
      <div class="modal-content small-modal danger-modal">
        ...
        <div class="modal-body">
          <div class="warn-text">⚠️ 以下操作不可撤销，请谨慎操作！建议仅在开发测试阶段使用。</div>
          
          <div class="maint-item">
            <h4>🧹 一键清空对局记录</h4>
            <p>清空所有巨人赛跑、点兵点将的对局记录和积分流水明细。<br>不影响用户余额和风控数据。</p>
            <button class="action-btn warn-btn" @click="resetBets">清空对局与流水</button>
          </div>

          <div class="maint-item">
            <h4>🧼 一键清空盈亏与风控</h4>
            <p>所有用户余额归零、重置风控特征、系统奖池归零。<br>对局记录保留。</p>
            <button class="action-btn error-btn" @click="resetProfits">清空盈亏与风控</button>
          </div>

          <div class="maint-item">
            <h4>☢️ 恢复出厂设置</h4>
            <p>清空一切数据（对局、流水、风控、余额），数据库恢复到刚部署状态。</p>
...
function filterUsers() { // computed 自动过滤
}
...
// ★ 新增：切换内部账号
async function toggleInternal(user) {
  try {
    const res = await authFetch('/api/admin/toggle-internal', {
      method: 'POST',
      body: JSON.stringify({ userId: user._id })
    })
    const data = await res.json()
...
async function toggleBan(user) {
  try {
    ...
    if (!user.banned) { body.reason = '管理员封禁' }
...
const canSubmitProfile = computed(() => {
  const nick = editProfileForm.value.nickname.trim()
  return nick.length >= 2 && nick.length <= 12
})

async function submitEditProfile() {
  if (!canSubmitProfile.value || submittingProfile.value) return
  submittingProfile.value = true
  try {
    const res = await authFetch('/api/admin/user-profile', {
      method: 'PUT',
      body: JSON.stringify({ userId: editProfileTarget.value._id, nickname: editProfileForm.value.nickname.trim(), avatar: editProfileForm.value.avatar })
    })
    const data = await res.json()
    if (res.ok) {
      toast('用户资料修改成功', 'success')
...
async function fetchSystemProfit() {
  try {
    ...
    const data = await res.json()
    if (res.ok) {
      systemProfit.value = data.currentProfit ?? data?.data?.currentProfit ?? 0
    }
  } catch (e) {
...
async function submitPoolAdjust() {
  ...
  try {
    const res = await authFetch('/api/admin/pool/adjust', {
      method: 'POST',
      body: JSON.stringify({ type: poolAdjustType.value, amount: poolAdjustAmount.value, remark: poolAdjustRemark.value })
    })
    const data = await res.json()
    if (res.ok) {
...
async function resetData() {
  if (!confirm('⚠️ 终极警告！确定要恢复出厂设置，清空一切数据吗？此操作不可逆！')) return
  try {
    const res = await authFetch('/api/admin/reset-data', { method: 'POST' })
    const data = await res.json()
    if (res.ok) {
      toast(data.message || '已重置', 'success')
      fetchUsers()
      fetchSystemProfit()
...
.admin-card:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,0.3); }
.highlight-card { border-color: rgba(231, 76, 60, 0.3); background: linear-gradient(135deg, #161b22 0%, #1e1520 100%); }
.danger-card { border-color: rgba(231, 76, 60, 0.4); background: linear-gradient(135deg, #161b22 0%, #1e1418 100%); }
...
.card-actions button { padding: 8px 20px; border-radius: 10px; border: none; cursor: pointer; font-size: 14px; font-weight: 600; background: rgba(255,255,255,0.08); color: #ccc; transition: background 0.2s; }
...
.danger-btn:hover { background: rgba(231,76,60,0.4) !important; }
.chat-badge { font-size: 12px; color: #e74c3c; font-weight: 600; background: rgba(231, 76, 60, 0.15); padding: 2px 8px; border-radius: 8px; }
...
.close-btn { width: 32px; height: 32px; border-radius: 50%; border: none; background: rgba(255,255,255,0.08); color: #888; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
.close-btn:hover { background: rgba(255,255,255,0.15); color: #fff; }
.modal-body { padding: 20px; overflow-y: auto; flex: 1; }

/* 搜索栏 */
.search-bar { display: flex; gap: 8px; margin-bottom: 16px; }
.search-bar input { flex: 1; background: #0d1117; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 8px 12px; color: #eee; font-size: 14px; outline: none; }
.search-bar input:focus { border-color: #f0c040; }
.search-bar button { padding: 8px 16px; border-radius: 8px; border: none; background: #f0c040; color: #000; font-weight: 600; cursor: pointer; }

/* 用户列表 */
.user-list { display: flex; flex-direction: column; gap: 8px; }
.user-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: rgba(255,255,255,0.03); border-radius: 10px; gap: 12px; flex-wrap: wrap; }
.user-info { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.user-avatar-small { width: 28px; height: 28px; border-radius: 50%; object-fit: cover; border: 2px solid rgba(255,255,255,0.15); }
.user-av-placeholder { font-size: 18px; }
.user-phone { font-weight: 600; font-size: 14px; }
.user-nickname { font-size: 13px; color: #f0f0f0; }
...
.user-internal { font-size: 11px; color: #f0c040; background: rgba(240,192,64,0.15); padding: 2px 6px; border-radius: 4px; font-weight: bold; }
.user-banned { font-size: 11px; color: #e74c3c; }
.user-actions { display: flex; gap: 6px; flex-wrap: wrap; }
.sm-btn { padding: 4px 10px; border-radius: 6px; border: none; font-size: 11px; cursor: pointer; font-weight: 600; background: rgba(255,255,255,0.08); color: #ccc; transition: background 0.15s; }
.sm-btn:hover { background: rgba(255,255,255,0.15); }
.adjust-btn { color: #f0c040; }
.profile-btn { color: #58a6ff; }
.internal-btn { color: #f0c040; border: 1px solid rgba(240,192,64,0.3); }
...
.cancel-btn { padding: 8px 20px; border-radius: 8px; border: none; background: rgba(255,255,255,0.08); color: #ccc; cursor: pointer; font-weight: 600; }
...
/* 修改资料表单 */
.edit-profile-form { display: flex; flex-direction: column; gap: 10px; }
.edit-profile-form label { font-size: 12px; color: #8b949e; margin-top: 6px; }
.edit-profile-form input { background: #0d1117; border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; padding: 8px 10px; color: #eee; font-size: 14px; outline: none; }
.edit-profile-form input:focus { border-color: #f0c040; }
.avatar-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; margin-top: 4px; }
...
.avatar-option.active { border-color: #f0c040; box-shadow: 0 0 0 2px rgba(240,192,64,0.4); }
...
.warn-text { background: rgba(243,156,18,0.1); color: #f39c12; padding: 10px 14px; border-radius: 8px; font-size: 13px; margin-bottom: 20px; border: 1px solid rgba(243,156,18,0.2); }
.maint-item { margin-bottom: 24px; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.05); }
.maint-item:last-child { border-bottom: none; margin-bottom: 0; }
.maint-item h4 { margin: 0 0 8px; font-size: 15px; color: #f0f0f0; }
.maint-item p { margin: 0 0 12px; font-size: 12px; color: #888; line-height: 1.5; }
.action-btn { padding: 8px 20px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; font-size: 13px; transition: opacity 0.2s; }
.action-btn:active { opacity: 0.8; }
.warn-btn { background: rgba(243,156,18,0.2); color: #f39c12; border: 1px solid rgba(243,156,18,0.3); }
.error-btn { background: rgba(231,76,60,0.2); color: #e74c3c; border: 1px solid rgba(231,76,60,0.3); }
...
.toast-item { padding: 10px 20px; border-radius: 10px; font-size: 14px; font-weight: 600; animation: slideIn 0.3s ease; }
