  ...
  <div class="admin-panel">
    ...
    <div class="card-grid">
      ...
      <div class="admin-card">
        ...
        <h3>用户管理</h3>
        <p>查看用户列表、余额调整、封禁管理、风控设置、修改用户资料</p>
          ...
          <div class="user-list">
            <div v-for="user in filteredUsers" :key="user._id" class="user-row">
              <div class="user-info">
                <img
                  v-if="user.avatar"
                  :src="AVATAR_MAP[user.avatar] || DEFAULT_AVATAR"
                ...
                <span
                  class="user-profit"
                  :class="{ positive: user.todayProfit > 0, negative: user.todayProfit < 0 }"
                >
      ...
      <div class="modal-content small-modal">
        ...
        <div class="modal-body">
          <div class="edit-profile-form">
            <label>UID（不可修改）</label>
            <input :value="editProfileTarget?.uid || '--'" disabled />

            <label>手机号（不可修改）</label>
            <input :value="editProfileTarget?.phone || '--'" disabled />
            ...
            <div class="avatar-grid">
              <div
                ...
                :key="n"
                class="avatar-option"
                :class="{ active: editProfileForm.avatar === n }"
                @click="editProfileForm.avatar = n"
              >
      ...
      <div class="modal-content small-modal">
        ...
        <div class="modal-body">
          <div class="pool-stat">
            ...
            <span
              class="pool-value"
              :class="{ positive: systemProfit > 0, negative: systemProfit < 0 }"
            >
...
async function submitEditProfile() {
  ...
  try {
    const res = await authFetch('/api/admin/user-profile', {
      method: 'PUT',
      body: JSON.stringify({
        userId: editProfileTarget.value._id,
...
.adjust-form input:not([type="number"]) {
  flex: 1;
  min-width: 120px;
}
