    ...
    <template #default>
      <div class="lobby-container" :style="backgroundStyle">
        ...
        <header class="lobby-header">
          ...
          <!-- 新增：设置按钮 -->
          <div class="header-actions" @click="showSettings = true"> ⚙️ </div>
        </header>
        <!-- Tab 内容区 -->
...
onMounted(async () => {
  ...
  heroes.forEach(name => {
    const img = new Image()
    img.src = `/assets/images/games2/${name}.png`
  })
...
// ✅ onUnmounted 必须放在最外层，和 onMounted 平级
onUnmounted(() => {
...
.header-actions { font-size: 20px; cursor: pointer; padding: 4px 8px; border-radius: 8px; transition: background 0.2s; }
.header-actions:active { background: rgba(212, 175, 55, 0.2); }
