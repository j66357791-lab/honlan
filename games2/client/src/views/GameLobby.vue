      ...
      <div class="lobby-container" :style="backgroundStyle">
        ...
        <header class="lobby-header">
          <div class="user-info">
            <img
              ...
              :src="AVATAR_MAP[currentUser.avatar]"
              class="header-avatar"
            />
          ...
          <div class="header-actions">
            <div
              class="action-btn"
              :class="{ 'has-badge': !!activeAnnouncement }"
        ...
        <div class="tab-content">
          ...
          <div v-show="activeTab === 'game'" class="tab-panel">
            ...
            <div class="game-card" @click="enterMatchGame">
              <div class="game-icon">🧩</div>
              <div class="game-info">
                <h3>自动消消乐</h3>
                <p>自动消除，策略干预</p>
          ...
          <div v-if="isAdmin" v-show="activeTab === 'reception'" class="tab-panel">
            ...
            <div v-if="receptionWaitingList.length" class="reception-list">
              ...
              <div
                v-for="s in receptionWaitingList"
                :key="s.id"
...
const receptionWaiting = computed(() =>
  receptionSessions.value.filter((s) => s.status === 'waiting').length
)
const receptionActive = computed(() =>
  receptionSessions.value.filter((s) => s.status === 'active').length
)
const receptionWaitingList = computed(() =>
  receptionSessions.value.filter((s) => s.status === 'waiting')
)
...
const enterMatchGame = () => router.push('/game/match')
...
function onAnnouncementClose({ noRemindToday }) {
  ...
  if (noRemindToday && activeAnnouncement.value) {
    dismissToday(activeAnnouncement.value._id)  // ← .id 改成 ._id
  }
}
...
.service-avatar {
  background: rgba(27, 176, 105, 0.12);
}
...
@supports (padding: max(0px)) {
  ...
  .tab-content {
    padding-bottom: calc(58px + max(8px, env(safe-area-inset-bottom)) + 10px);
  }
}
</style>
