<template>
  <div class="game-app" :class="{ shaking: screenShaking }">
    ...
    <div class="tool-bar">
      ...
      <button class="tool-btn" @click="handleLogout">🚪 退出</button>
    </div>
    ...
    <div class="bet-container" v-if="gamePhase !== 'racing'">
      <BetPanel :choice="choice" :bet-amount="betAmount" :balance="balance" :is-insufficient="isInsufficient" :game-phase="gamePhase" :can-bet="canBet" @select="selectChoice" @set-amount="setBetAmount" @quick-bet="quickBet" @place-bet="placeBet" />
    </div>
    ...
    <TrendModal :visible="showTrend" :history="history" @close="showTrend = false" />
  </div>
</template>
...
import TrendModal from '../components/TrendModal.vue' // 新增走势图组件
...
const showTrend = ref(false) // 控制走势图弹窗
