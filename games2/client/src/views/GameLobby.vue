<template> 
  <div class="lobby-container" :style="backgroundStyle"> 
    ...
    <header class="lobby-header"> 
      ...
      <!-- 新增：设置按钮 -->
      <div class="header-actions" @click="showSettings = true">
        ⚙️
      </div>
    </header> 
    ...
    <div class="tab-content"> 
      ...
      <div v-if="isAdmin" v-show="activeTab === 'reception'" class="tab-panel"> 
        ...
        <button class="reception-enter" @click="router.push('/admin/chat')"> 进入工作台 › </button> 
      </div> 

      <!-- ====== 活动 ====== --> 
      <!-- 修改：使用独立的 ActivityCenter 组件 -->
      <div v-show="activeTab === 'activity'" class="tab-panel no-padding">
        <ActivityCenter />
      </div>

      <!-- ====== 用户：我的 ====== --> 
      <ProfilePanel v-if="!isAdmin" v-show="activeTab === 'profile'" :current-user="currentUser" :is-admin="isAdmin" :display-balance="displayBalance" @logout="handleLogout" @go-admin="goToAdmin" @go-chat-admin="goToChatAdmin" /> 
      ...
      <div v-if="isAdmin" v-show="activeTab === 'admin'" class="tab-panel"> 
        ...
        <button class="admin-enter" @click="router.push('/admin')"> 进入后台 › </button> 
    ...
    <nav class="bottom-nav"> 
      <div v-for="tab in displayTabs" :key="tab.key" class="nav-item" :class="{ active: activeTab === tab.key }" @click="onTabClick(tab.key)" > 
        <span class="nav-icon">{{ activeTab === tab.key ? tab.activeIcon : tab.icon }}</span> 
        <span class="nav-label">{{ tab.label }}</span> 
        <!-- 用户：消息红点 --> 
        <span v-if="tab.key === 'message' && !isAdmin && unreadCount > 0" class="nav-badge" >{{ unreadCount }}</span> 
        <!-- 管理员：接待台红点（等待数 + 未读消息数） --> 
        <span v-if="tab.key === 'reception' && isAdmin && (receptionWaiting > 0 || unreadCount > 0)" class="nav-badge" :class="{ 'dot-only': receptionWaiting === 0 && unreadCount > 0 }" >{{ receptionWaiting > 0 ? receptionWaiting : '' }}</span> 
      </div> 
    </nav> 
...
const backgroundStyle = computed(() => ({ backgroundImage: `url('https://t1.chatglm.cn/file/6a0c13cb6bd5e5d2b3b9a11c.png?expired_at=1779608488&sign=5eee1da0856a15bf73eac639576ff037&ext=png')`, backgroundSize: 'cover', backgroundPosition: 'center', minHeight: '100vh' })) 
...
<style scoped> 
.lobby-container { min-height: 100vh; color: #333; padding: 0; display: flex; flex-direction: column; position: relative; overflow: hidden; } 
...
.tab-panel.no-padding { padding: 0; } /* 活动组件自带padding */
.tab-panel h2 { font-size: 20px; color: #d4af37; margin-bottom: 16px; text-shadow: 0 1px 2px rgba(0,0,0,0.1); } 
.game-card { width: 100%; max-width: 400px; margin: 0 auto 16px; background: rgba(255, 248, 220, 0.8); border: 1px solid rgba(212, 175, 55, 0.3); border-radius: 12px; padding: 20px; display: flex; align-items: center; gap: 15px; cursor: pointer; transition: all 0.2s; backdrop-filter: blur(5px); box-shadow: 0 2px 8px rgba(0,0,0,0.05); } 
...
.bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; display: flex; height: 58px; background: rgba(255, 248, 220, 0.92); backdrop-filter: blur(12px); border-top: 1px solid rgba(212, 175, 55, 0.2); z-index: 100; } 
.nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; cursor: pointer; position: relative; } 
