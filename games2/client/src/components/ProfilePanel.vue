<template>
  <div class="profile-panel">
    ...
    <div class="profile-card">
      ...
      <div class="profile-main">
        <div class="name-row">
          ...
          <!-- ★ 优化：VIP 徽章动态计算 -->
          <span class="vip-badge" :class="vipBadgeClass">{{ vipBadgeText }}</span>
    ...
    <div class="balance-card">
      <div class="balance-row">
        ...
        <div class="balance-item pos-rel">
          ...
          <div class="balance-value num-font crystal-color">{{ ((displayCrystal || 0) / 1000).toFixed(2) }}</div>
          <div class="balance-unit">晶石</div>
          <button class="crystal-transfer-btn" @click="openTransferModal">转增</button>
          <button class="crystal-record-btn" @click="$router.push('/transfer-records')">记录</button>
        </div>
      </div>
    </div>
    ...
    <div class="menu-group">
      ...
      <!-- ★ 新增：月卡中心入口 -->
      <div class="menu-item highlight-item" @click="$router.push('/membership')">
        <span class="menu-icon">👑</span>
        <span class="menu-text">月卡中心</span>
        <span class="menu-tag" style="background:#8a2be2">VIP</span>
        <span class="menu-arrow">›</span>
      </div>
...
const { updateNickname, updateAvatar, getNicknameCooldownDays, refreshUser, displayBalance, displayCrystal, updateCrystal } = useAuth()
...
// ★ 优化：VIP徽章动态计算逻辑（增加过期时间校验，防止后端未重置type导致的前端显示错误）
const isActiveMember = computed(() => {
  if (!props.currentUser) return false
  const type = props.currentUser.membershipType
  const expireAt = props.currentUser.membershipExpireAt
  // 类型不是none，且有到期时间，且到期时间大于当前时间
...
const vipBadgeClass = computed(() => {
  // 优先判断是否有效，无效统一显示灰色
  if (!isActiveMember.value) return 'no-vip'
  // 有效则判断等级
  if (props.currentUser.membershipType === 'super') return 'is-svip'
  return 'is-vip'
})

const vipBadgeText = computed(() => {
  if (!isActiveMember.value) return 'VIP'
  if (props.currentUser.membershipType === 'super') return 'SVIP'
  return 'VIP'
})
...
async function selectAvatar(n) {
  ...
  try { await updateAvatar(n); showAvatarPicker.value = false } catch (err) { alert(err.message || '头像修改失败') }
}
...
async function submitNickname() {
  ...
  try { await updateNickname(newNickname.value.trim()); showNicknameEditor.value = false; await refreshUser() } 
  catch (err) { if (err.message.includes('还需等待')) { const match = err.message.match(/还需等待(\d+)天/); if (match) cooldownDays.value = Number(match[1]) } alert(err.message || '昵称修改失败') } 
...
function openTransferModal() { showTransferModal.value = true; resetTransferModal() }
function resetTransferModal() { targetInfo.value = null; transferAccount.value = ''; transferAmount.value = '' }

async function checkTargetUser() {
  if (!transferAccount.value.trim()) return alert('请输入对方UID或手机号')
  checking.value = true
  try { const res = await request('/api/transfer/check-target', { method: 'POST', body: JSON.stringify({ account: transferAccount.value.trim() }) }); const data = await res.json(); targetInfo.value = data } 
  catch (err) { alert(err.message || '查询失败') } 
...
const transferFee = computed(() => { if (validAmount.value <= 0) return '0.00'; return (validAmount.value * 0.05).toFixed(2) })
const transferTotalCost = computed(() => { if (validAmount.value <= 0) return '0.00'; const total = validAmount.value + parseFloat(transferFee.value); return total.toFixed(2) })
const canTransfer = computed(() => { return validAmount.value >= 1 && parseFloat(transferTotalCost.value) <= (displayCrystal.value / 1000) })

async function submitTransfer() {
  if (!canTransfer.value) return alert('余额不足或数量错误')
  transferring.value = true
  try {
    const res = await request('/api/transfer/crystal', { method: 'POST', body: JSON.stringify({ targetAccount: targetInfo.value.uid, amount: validAmount.value }) })
    const data = await res.json()
    alert(`转增成功！订单号: ${data.tradeNo}`)
    showTransferModal.value = false
    if (data.crystal !== undefined) { updateCrystal(data.crystal) }
    await refreshUser()
  } catch (err) { alert(err.message || '转增失败') } 
...
/* 未开通/已过期：灰色 */
.no-vip { background: #eee; color: #bbb; }
/* 普通VIP：金色闪耀 */
.is-vip { 
  background: linear-gradient(135deg, #d4af37, #f7df7e, #d4af37);
...
/* 超级SVIP：紫金炫酷流光 + 呼吸放大 */
.is-svip { 
  background: linear-gradient(135deg, #8a2be2, #ff6ec7, #ffd700, #8a2be2);
  background-size: 300% 300%; 
  color: #fff; 
  font-size: 11px; 
  animation: svipShine 2s ease infinite, svipPulse 1.5s ease-in-out infinite;
  box-shadow: 0 0 8px rgba(138, 43, 226, 0.6), 0 0 15px rgba(255, 110, 199, 0.4);
  text-shadow: 0 0 4px rgba(255, 255, 255, 0.8);
}
@keyframes vipShine { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
@keyframes svipShine { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
@keyframes svipPulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }
...
.crystal-transfer-btn { position: absolute; top: -4px; right: -4px; background: #8a2be2; color: #fff; border: none; font-size: 10px; padding: 2px 6px; border-radius: 6px; cursor: pointer; font-weight: bold; }
.crystal-record-btn { position: absolute; top: -4px; left: -4px; background: #d4af37; color: #fff; border: none; font-size: 10px; padding: 2px 6px; border-radius: 6px; cursor: pointer; font-weight: bold; }

.transfer-modal { max-width: 360px; }
.transfer-step { width: 100%; }
.target-info-card { background: rgba(138, 43, 226, 0.08); border: 1px dashed rgba(138, 43, 226, 0.3); border-radius: 8px; padding: 12px; margin-bottom: 12px; text-align: center; }
.target-confirm-row { margin-bottom: 8px; }
.target-name { font-size: 18px; font-weight: 700; color: #333; margin-bottom: 4px; }
.target-uid { font-size: 13px; color: #8a2be2; font-weight: 600; letter-spacing: 0.5px; }
.target-warning { font-size: 11px; color: #e74c3c; text-align: center; margin-top: 8px; padding-top: 8px; border-top: 1px dashed rgba(138, 43, 226, 0.2); }
.check-btn { width: 100%; margin-top: 10px; }
.transfer-fee-hint { font-size: 12px; color: #888; margin-top: 8px; line-height: 1.8; padding: 8px; background: #fffcf0; border-radius: 6px; border: 1px solid rgba(212, 175, 55, 0.2); }
