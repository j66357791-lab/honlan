...
// ★ 修复：解构出 updateCrystal 方法，用于转增成功后立即更新余额
...
async function checkTargetUser() {
  ...
  checking.value = true
  try {
    const res = await request('/api/transfer/check-target', { method: 'POST', body: JSON.stringify({ account: transferAccount.value.trim() }) })
    const data = await res.json()
    targetInfo.value = data
  } catch (err) { alert(err.message || '查询失败') } finally { checking.value = false }
}
...
// ★ 修复：转增成功后，立即调用 updateCrystal 更新全局状态
async function submitTransfer() {
  ...
  try {
    ...
    // 立即用后端返回的最新晶石数值更新全局状态，页面秒变！
    if (data.crystal !== undefined) {
      updateCrystal(data.crystal)
    }
    // 再拉一次全量用户信息兜底
    await refreshUser()
  } catch (err) { alert(err.message || '转增失败') } finally { transferring.value = false }
}
</script>
