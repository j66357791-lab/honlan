<template>
  <div class="pointing-bet-panel" ref="panelRef" :style="{ '--chip-size': chipSize + 'px' }">
    <!-- 上横条插槽 -->
    <div class="top-bar-slot"><slot name="top-bar"></slot></div>
    <!-- 左侧英雄区域插槽 -->
    <div class="side-column left-column"><slot name="left-heroes" v-bind="slotProps"></slot></div>
    
    <!-- 中间操作区 -->
    <div class="center-area">
      <div class="betting-controls">
        <div class="action-area">
          <div class="total-bet">总计: <span>{{ totalBetAmount >= 1000 ? (totalBetAmount/1000) + 'w' : totalBetAmount }}</span></div>
        ...
        <div class="chip-buttons">
          <button v-for="chip in chipValues" :key="chip" :data-chip="chip" class="chip-btn"
            ...
            <div class="chip-inner-circle">
              ...
              <span class="chip-amount">{{ chip >= 1000 ? (chip/1000) + 'w' : chip }}</span>
            </div>
          </button>
        </div>
      </div>
    ...
    <!-- 右侧英雄区域插槽 -->
    <div class="side-column right-column"><slot name="right-heroes" v-bind="slotProps"></slot></div>
    <!-- 下横条插槽 -->
    <div class="bottom-bar-slot"><slot name="bottom-bar"></slot></div>
    ...
    <div v-for="fly in flyingChips" :key="fly.id" class="flying-chip"
      ...
      <div class="chip-bg-pattern"></div>
      <span class="flying-amount">{{ fly.amount >= 1000 ? (fly.amount/1000) + 'w' : fly.amount }}</span>
    </div>
  </div>
</template>
...
// ★ 新增限额常量
const MAX_BET_NORMAL = 200000
const MAX_BET_SPECIAL = 50000
const MAX_TOTAL_BET = 500000
...
const slotProps = computed(() => ({
  characters: props.characters, bets: characterBets, animatingEliminated: props.animatingEliminated,
  result: props.result, highlightIndex: props.highlightIndex, handlePlaceBet, handleRevokeBet
}))
...
async function handlePlaceBet(name, event) {
  ...
  // ★ 新增：单英雄限额校验
  const charInfo = props.characters.find(c => c.name === name)
  const maxBet = charInfo && charInfo.tier === 'normal' ? MAX_BET_NORMAL : MAX_BET_SPECIAL
  if (characterBets[name] + amount > maxBet) {
    alert(`${name}单注总额不能超过 ${(maxBet / 10000)}万`)
    return
  }
  
  // ★ 新增：单局总限额校验
  if (totalBetAmount.value + amount > MAX_TOTAL_BET) {
    alert(`单局总下注不能超过 ${(MAX_TOTAL_BET / 10000)}万`)
    return
  }
...
function handleStartGame() {
  ...
  // ★ 最终校验
  if (totalBetAmount.value > MAX_TOTAL_BET) {
    alert(`单局总下注不能超过 ${(MAX_TOTAL_BET / 10000)}万`)
    return
  }
  const bets = []
...
<style scoped>
.pointing-bet-panel { --chip-size: 60px; --grid-col-side: 130px; --grid-row-bar: 65px; --grid-gap: 15px; position: relative; display: grid; grid-template-columns: var(--grid-col-side) 1fr var(--grid-col-side); grid-template-rows: var(--grid-row-bar) 1fr var(--grid-row-bar); gap: var(--grid-gap); width: 100%; max-width: 750px; margin: 0 auto; }
.top-bar-slot { grid-column: 1 / 4; grid-row: 1; }
.bottom-bar-slot { grid-column: 1 / 4; grid-row: 3; }
.side-column { grid-row: 2; display: flex; flex-direction: column; justify-content: space-around; gap: 10px; }
.left-column { grid-column: 1; }
.right-column { grid-column: 3; }
.center-area { grid-column: 2; grid-row: 2; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 25px; min-height: 280px; }
...
.start-game-btn { width: 100%; padding: 14px; border: none; border-radius: 8px; background: linear-gradient(135deg, #ffd700, #ff8c00); color: #000; font-weight: bold; font-size: 16px; cursor: pointer; }
...
@media (max-width: 480px) { .pointing-bet-panel { --grid-col-side: 80px; --grid-row-bar: 45px; --grid-gap: 8px; } .center-area { min-height: 220px; gap: 15px; } .chip-section { max-width: 100%; } }
</style>
