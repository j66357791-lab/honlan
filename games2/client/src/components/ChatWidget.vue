    ...
    <div class="chat-fullscreen">
      ...
      <div v-else class="chat-body" ref="chatBodyEl">
        <div v-for="msg in messages" :key="msg.id" class="msg-row"
          :class="{ 'msg-self': msg.sender === 'user', 'msg-other': msg.sender === 'admin' || msg.sender === 'bot', 'msg-system': msg.sender === 'system' }">
          ...
          <template v-else-if="msg.sender === 'admin' || msg.sender === 'bot'">
            <div class="avatar" :class="msg.sender === 'bot' ? 'bot-av' : 'admin-av'">
              <img v-if="msg.senderAvatar" :src="AVATAR_MAP[msg.senderAvatar] || DEFAULT_AVATAR" class="avatar-img" />
              <span v-else>{{ msg.sender === 'bot' ? '🤖' : '🧑‍💼' }}</span>
            </div>
          ...
          <template v-else>
            ...
            <div class="avatar self-av">
              <img v-if="currentUser?.avatar" :src="AVATAR_MAP[currentUser.avatar] || DEFAULT_AVATAR" class="avatar-img" />
              <span v-else>😊</span>
            </div>
          </template>
        </div>
      ...
      <div class="chat-input-bar" v-if="sessionStatus !== 'closed'">
        <input v-model="inputText" @keydown.enter="send" placeholder="输入消息或关键词..." :disabled="!connected" />
...
const { connected, messages, sessionStatus, currentSessionId, connect: connectChat, sendMessage, markRead, resetSession } = useChat()
...
<style scoped>
.chat-fullscreen { position: fixed; inset: 0; background: #f5f5f5; z-index: 200; display: flex; flex-direction: column; }
.chat-topbar { display: flex; align-items: center; justify-content: space-between; padding: 14px 12px; background: #fff; border-bottom: 1px solid #e8e8e8; flex-shrink: 0; }
.chat-back { width: 44px; height: 44px; border-radius: 50%; border: none; background: transparent; color: #333; font-size: 22px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
.quick-area { flex: 1; padding: 30px 20px; display: flex; flex-direction: column; align-items: center; }
.quick-greeting { font-size: 16px; color: #333; margin-bottom: 28px; text-align: center; }
.quick-list { width: 100%; max-width: 320px; display: flex; flex-direction: column; gap: 10px; }
.quick-btn { width: 100%; padding: 14px 16px; border-radius: 12px; border: 1px solid #e0e0e0; background: #fff; color: #333; font-size: 15px; cursor: pointer; text-align: left; transition: background 0.15s; }
...
.avatar { width: 36px; height: 36px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; overflow: hidden; }
...
.bubble { padding: 10px 14px; border-radius: 8px; font-size: 14px; line-height: 1.6; word-break: break-word; white-space: pre-line; }
.other-bubble { background: #fff; color: #333; border-top-left-radius: 2px; box-shadow: 0 1px 2px rgba(0,0,0,0.04); }
.bot-bubble { background: #e8f0fe; color: #333; border-top-left-radius: 2px; }
.self-bubble { background: #1bb069; color: #fff; border-top-right-radius: 2px; }
.bubble-time { font-size: 10px; color: #bbb; margin-top: 3px; padding: 0 4px; }
.bubble-time.right { text-align: right; }
.sys-msg { font-size: 12px; color: #999; padding: 4px 14px; background: rgba(0,0,0,0.03); border-radius: 12px; }
.waiting-hint { display: flex; align-items: center; gap: 8px; justify-content: center; color: #999; font-size: 13px; padding: 8px 0; }
.dots { display: flex; gap: 4px; }
.dots span { width: 5px; height: 5px; border-radius: 50%; background: #bbb; animation: dotBounce 1.4s infinite both; }
.dots span:nth-child(1) { animation-delay: 0s; }
.dots span:nth-child(2) { animation-delay: 0.2s; }
.dots span:nth-child(3) { animation-delay: 0.4s; }
@keyframes dotBounce { 0%, 80%, 100% { transform: scale(0.6); opacity: 0.4; } 40% { transform: scale(1); opacity: 1; } }
.closed-area { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 20px 0; }
.closed-text { font-size: 12px; color: #bbb; }
.reconsult-btn { padding: 10px 32px; border-radius: 22px; border: 1px solid #1bb069; background: #fff; color: #1bb069; font-size: 14px; font-weight: 600; cursor: pointer; }
.reconsult-btn:active { background: #f0faf5; }
.chat-input-bar { display: flex; gap: 8px; padding: 10px 12px; background: #fff; border-top: 1px solid #e8e8e8; flex-shrink: 0; }
.chat-input-bar input { flex: 1; background: #f5f5f5; border: 1px solid #e0e0e0; border-radius: 20px; padding: 10px 16px; color: #333; font-size: 14px; outline: none; }
.chat-input-bar input:focus { border-color: #1bb069; }
.chat-input-bar input::placeholder { color: #bbb; }
.send-btn { padding: 0 22px; border-radius: 20px; border: none; background: #1bb069; color: #fff; font-size: 14px; font-weight: 600; cursor: pointer; }
.send-btn:active:not(:disabled) { background: #17a05c; }
.send-btn:disabled { opacity: 0.3; cursor: not-allowed; }
.slide-right-enter-active, .slide-right-leave-active { transition: transform 0.28s cubic-bezier(0.4, 0, 0.2, 1); }
