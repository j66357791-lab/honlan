...
router.post('/check-target', authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ $or: [{ uid: account }, { phone: account }] }).select('uid nickname');
...
router.post('/crystal', authMiddleware, transferLimiter, async (req, res) => {
  ...
  try {
    ...
    const transferAmountRaw = parseFloat(amount);
    // 校验：必须是数字，最小1起转
    if (isNaN(transferAmountRaw) || transferAmountRaw < 1) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({ error: '转增数量必须是大于等于1的数字' });
    }
    // 校验：最多允许两位小数
    const decimalPart = transferAmountRaw.toString().split('.')[1];
    ...
    // ★★★ 核心修复：将前端传入的真实金额，转换为数据库内部放大1000倍的整数单位 ★★★
    const transferAmount = Math.round(transferAmountRaw * 1000);
    
    // 2. 计算手续费 (5%，基于内部整数计算，自动四舍五入到内部整数，即精确到小数点后3位)
    const fee = Math.round(transferAmount * 0.05);
    ...
    if (!fromUser) {
      ...
      // 返回给前端时，除以1000转回真实单位显示
      return res.status(400).json({ error: `晶石不足，需支付${(totalCost/1000).toFixed(2)}(含手续费${(fee/1000).toFixed(2)})` });
    }
    ...
    // 6. 记录流水 (金额字段也存放大1000倍的整数，与其他流水保持一致)
    await Transaction.create([
      {
        ...
        currency: 'crystal',
        remark: `转增给UID:${toUser.uid} ${(transferAmount/1000).toFixed(2)}晶石 (手续费${(fee/1000).toFixed(2)})`,
        gameType: 'transfer',
        tradeNo
      },
      {
    ...
    ], { session, ordered: true }); // 修复：事务内批量插入必须加 ordered: true

    // 7. 提交事务
    await session.commitTransaction();
    session.endSession();

    // 返回给前端的晶石余额，也要除以1000转回真实单位
    res.json({ 
      message: '转增成功', 
      crystal: fromUser.crystal, // 直接返回内部数值，前端统一处理格式化
      fee, 
      tradeNo 
    });
  } catch (err) {
...
router.get('/records', authMiddleware, async (req, res) => {
  try {
    ...
    const [list, total] = await Promise.all([
      Transaction.find({ userId, gameType: 'transfer' })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(pageSize)
        .select('type amount fee currency remark tradeNo createdAt')
        .lean(),
      Transaction.countDocuments({ userId, gameType: 'transfer' })
    ]);
    ...
    console.error('[转增记录查询错误]', err);
    res.status(500).json({ error: '查询转增记录失败' });
  }
});
