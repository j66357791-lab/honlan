...
// ★ 修改：区分普通和特殊英雄的单注上限
const POINTING_MAX_BET_NORMAL = 200000;  // 5倍普通英雄单注上限 20w
const POINTING_MAX_BET_SPECIAL = 50000;  // 10/20/30/40倍特殊英雄单注上限 5w
const POINTING_MAX_TOTAL = 500000;       // ★ 修改：单局总下注上限 50w
...
const BASE_PROB_MAP = {
  '赵云': 0.20, '秦良玉': 0.20, '马超': 0.20, '花木兰': 0.20,
  '关羽': 0.08, '张飞': 0.05, '梁红玉': 0.03, '穆桂英': 0.02,
  'all_eliminated': 0.01, 'all_survived': 0.01
};
...
router.post('/bet', authMiddleware, betLimiter, async (req, res) => {
  ...
  try {
    ...
    for (const b of bets) {
      ...
      if (isNaN(amt) || amt < POINTING_MIN_BET) {
        await session.abortTransaction(); session.endSession();
        return res.status(400).json({ error: `单注金额最低为${POINTING_MIN_BET}` });
      }
      ...
      const heroInfo = CHARACTERS.find(c => c.name === b.choice);
      const currentMaxBet = heroInfo && heroInfo.tier === 'normal' ? POINTING_MAX_BET_NORMAL : POINTING_MAX_BET_SPECIAL;
      
      if (amt > currentMaxBet) {
        await session.abortTransaction(); session.endSession();
        return res.status(400).json({ error: `${b.choice}单注金额不能超过${currentMaxBet.toLocaleString()}` });
      }
    ...
    if (totalAmount > POINTING_MAX_TOTAL) {
      await session.abortTransaction(); session.endSession();
      return res.status(400).json({ error: `单局总下注额不能超过${POINTING_MAX_TOTAL.toLocaleString()}` });
    }
    ...
    const user = await User.findOneAndUpdate(
      ...
      { $inc: { balance: -totalAmount, totalBetAmount: Math.floor(totalAmount * TURNOVER_RATE.pointing) } },
      { new: true, session }
    );
    ...
    if (isAllSame && allHeroFactors[0] !== 1.0) {
      ...
      for (const choice of userChoices) {
        if (finalProbMap[choice] !== undefined) finalProbMap[choice] *= factor;
      }
      const unchosenKeys = heroKeys.filter(k => !userChoices.includes(k));
      const reverseFactor = 1.0 / factor;
      for (const k of unchosenKeys) {
        if (finalProbMap[k] !== undefined) finalProbMap[k] *= reverseFactor;
      }
      if (finalProbMap['all_survived'] !== undefined) finalProbMap['all_survived'] *= factor;
      if (finalProbMap['all_eliminated'] !== undefined) finalProbMap['all_eliminated'] *= reverseFactor;
      
      if (initialModifier.special !== undefined) {
        const specialFactor = initialModifier.special / factor;
        for (const hero of ['关羽', '张飞', '梁红玉', '穆桂英']) {
          if (finalProbMap[hero] !== undefined) finalProbMap[hero] *= specialFactor;
        }
      }
    } else {
    ...
    if (initialModifier.emptyWeightFactor && initialModifier.emptyWeightFactor > 1) {
      ...
      for (const ek of emptyKeys) finalProbMap[ek] *= ewf;
    }
    ...
    if (userNetLoss > 10000 && maxPotentialPayout <= userNetLoss * 0.5) {
      finalProbMap['穆桂英'] *= 1.5; finalProbMap['梁红玉'] *= 1.5; finalProbMap['all_survived'] *= 2.0;
    } else if (userNetLoss <= 0 && coverageCount >= 6) {
      finalProbMap['穆桂英'] = 0; finalProbMap['梁红玉'] = 0; finalProbMap['张飞'] = 0; finalProbMap['all_survived'] = 0; finalProbMap['all_eliminated'] *= 5;
    }
    ...
    if (eatPhase === 'FEED' && gameImpact < 0) {
      ...
      for (const choice of userChoices) {
        if (finalProbMap[choice] !== undefined) finalProbMap[choice] *= (1.0 + feedBoost * 0.5);
      }
    ...
    if (totalProb <= 0) {
      finalProbMap['赵云'] = 0.25; finalProbMap['秦良玉'] = 0.25; finalProbMap['马超'] = 0.25; finalProbMap['花木兰'] = 0.25;
    } else {
    ...
    // 5. 抽奖
    // ==========================================
    const random = crypto.randomInt(0, 100000) / 100000;
    ...
    await Transaction.create([{ userId, type: 'bet_expense', amount: totalAmount, balanceAfter: balanceAfterBet, remark: `点兵下注 ${totalAmount} 积分`, gameType: 'pointing' }], { session });
    if (totalPayout > 0) {
      await Transaction.create([{ userId, type: 'bet_win', amount: totalPayout, balanceAfter: balanceAfterBet + totalPayout, remark: `点兵赢取 ${totalPayout} 积分 (服务费:${feeDeducted})`, gameType: 'pointing' }], { session });
    }

    await PointingBet.create([{ userId, roundId: crypto.randomUUID(), bets: bets.map(b => ({ choice: b.choice, amount: b.amount })), totalAmount, resultType, survivedCharacters, totalPayout, netChange }], { session });

    await session.commitTransaction();
    session.endSession();

    riskEngine.settle(userId, 'pointing_game', totalAmount, totalPayout, { choices: userChoices, coverageRate: coverageCount / 8 }).catch(() => {});
...
router.get('/history', authMiddleware, async (req, res) => {
  try {
    ...
    const records = await PointingBet.find({ userId: req.user.userId })
      .sort({ createdAt: -1 })
      .limit(limit)
      .select('survivedCharacters resultType createdAt');
...
router.get('/transactions', authMiddleware, async (req, res) => {
  try {
    const limit = Math.min(parseInt(req.query.limit) || 20, 50);
    const list = await Transaction.find({ userId: req.user.userId, gameType: 'pointing' }).sort({ createdAt: -1 }).limit(limit);
    res.json({ list });
  } catch (err) {
