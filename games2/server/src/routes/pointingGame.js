...
router.get('/characters', authMiddleware, (req, res) => { 
  try { 
    ...
    res.json({ characters: charList, specialEvents: { all_eliminated: BASE_PROB_MAP.all_eliminated, all_survived: BASE_PROB_MAP.all_survived }, feeRate: SERVICE_FEE }); 
  } catch (err) { 
...
router.post('/bet', authMiddleware, betLimiter, async (req, res) => { 
  ...
  try { 
    ...
    // ==========================================
    // 2. 原子扣款（带事务）—— 🔴🔴🔴 仅此处修改，加了 totalBetAmount 🔴🔴🔴
    // ==========================================
    ...
    // 5. 抽奖
    // ==========================================
    const random = crypto.randomInt(0, 100000) / 100000; // 生成 [0, 1) 的浮点数
    ...
    // ★ 不返回 _risk 风控信息
    res.json({ message: totalPayout > 0 ? '恭喜获胜！' : '很遗憾未中', result: { resultType, survivedCharacters }, totalPayout, feeDeducted, netChange, balance: balanceAfterBet + totalPayout, balanceAfter: balanceAfterBet + totalPayout }); 
  } catch (err) { 
...
router.get('/history', authMiddleware, async (req, res) => { 
  try { 
    ...
    const records = await PointingBet.find({ userId: req.user.userId }) 
      .sort({ createdAt: -1 }) 
      .limit(limit) 
      // 🚀 核心优化：只查询走势图需要的3个字段，数据体积缩小 80%，速度起飞！
