...
const CHARACTERS = [
  { name: '赵云', gender: 'male', power: 4, odds: 2.7 }, { name: '关羽', gender: 'male', power: 3, odds: 3.4 },
  { name: '张飞', gender: 'male', power: 2, odds: 4.9 }, { name: '马超', gender: 'male', power: 1, odds: 8.0 },
  { name: '秦良玉', gender: 'female', power: 4, odds: 2.7 }, { name: '梁红玉', gender: 'female', power: 3, odds: 3.4 },
  { name: '穆桂英', gender: 'female', power: 2, odds: 4.9 }, { name: '花木兰', gender: 'female', power: 1, odds: 8.0 }
];
...
function generateBasePointingResult() {
  ...
  while (survived.length > survivalCount) {
    ...
    const rand = crypto.randomInt(totalPower); let acc = 0;
    for (let i = 0; i < survived.length; i++) { acc += survived[i].power; if (rand < acc) { survived.splice(i, 1); break; } }
  }
...
function applyRiskControl(baseResult, riskAction, bets) {
  ...
  let surviveGender;
  if (riskAction === 'FORCE_LOSE') {
    surviveGender = betGenders.includes('male') ? 'female' : 'male';
  } else if (riskAction === 'FORCE_WIN') {
    surviveGender = betGenders.length > 0 ? betGenders[0] : CHARACTERS.find(c => betNames.includes(c.name))?.gender || 'male';
  }

  const genderCharacters = CHARACTERS.filter(c => c.gender === surviveGender);
  const survived = [...genderCharacters];
  
  if (riskAction === 'FORCE_WIN' && betNames.length > 0) {
    while (survived.length > 1) {
      const nonBetChars = survived.filter(c => !betNames.includes(c.name));
      if (nonBetChars.length > 0) { survived.splice(survived.indexOf(nonBetChars[0]), 1); } else break;
    }
  } else {
    while (survived.length > 2) { survived.splice(crypto.randomInt(survived.length), 1); }
  }
...
router.post('/bet', authMiddleware, async (req, res) => {
  try {
    ...
    let totalAmount = 0;
    for (const b of bets) { const amt = parseInt(b.amount); if (isNaN(amt) || amt <= 0) return res.status(400).json({ error: '单注金额必须为大于0的整数' }); b.amount = amt; totalAmount += amt; }
    ...
    if (!user) { const c = await User.findById(userId); if (!c) return res.status(404).json({ error: '用户不存在' }); if (c.banned) return res.status(403).json({ error: '账号已被封禁' }); return res.status(400).json({ error: '积分不足' }); }
    ...
    bets.forEach(bet => {
      let winAmount = 0;
      if (gameResult.resultType === 'all_survived') { winAmount = bet.type === 'gender' ? bet.amount * GENDER_ODDS : bet.amount * CHARACTERS.find(c => c.name === bet.choice).odds; }
      else if (gameResult.resultType === 'normal') {
        if (bet.type === 'gender') { const wg = CHARACTERS.find(c => gameResult.survivedCharacters.includes(c.name))?.gender; if (bet.choice === wg) winAmount = bet.amount * GENDER_ODDS; }
        else if (gameResult.survivedCharacters.includes(bet.choice)) { winAmount = bet.amount * CHARACTERS.find(c => c.name === bet.choice).odds; }
      }
    ...
    if (totalPayout > 0) { await User.updateOne({ _id: userId }, { $inc: { balance: totalPayout } }); user.balance += totalPayout; }
    ...
    const betRecord = await PointingBet.create({ userId, bets, totalAmount, resultType: gameResult.resultType, survivedCharacters: gameResult.survivedCharacters, totalPayout, netChange });
    await Transaction.create({ userId, type: 'bet_expense', amount: totalAmount, balanceAfter: user.balance - totalPayout, remark: `点兵点将下注 ${totalAmount}积分`, betId: betRecord._id });
    if (totalPayout > 0) { await Transaction.create({ userId, type: 'bet_win', amount: totalPayout, balanceAfter: user.balance, remark: `点兵点将赢取 ${totalPayout}积分`, betId: betRecord._id }); }

    res.json({ result: gameResult, payout: totalPayout, netChange, balance: user.balance });
  } catch (err) { console.error(`[点兵点将] ${err.message}`); res.status(500).json({ error: '下注失败' }); }
});
