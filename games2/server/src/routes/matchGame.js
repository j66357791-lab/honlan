...
import appLogger from '../utils/logger.js'; // ★ 引入日志记录耗时

const router = Router();

// ========== 游戏配置 ==========
const ALLOWED_PRICES = [5000, 20000, 100000, 500000]; 
const BOARD_SIZE = 8;
const ELEMENT_TYPES = 5;
const BASE_SCORE = 2.0;
const SCORE_MULTIPLIER = 4.0;
const FEE_RATE = 0.10;
const MAX_WAVES = 50; // ★★★ 新增：防止极端连消卡死服务器
...
router.post('/bet', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 1. 扣款 (必须事务内) - 关闭 runValidators 提速
    ...
    appLogger.info(`[消消乐-耗时2] 取棋盘+计算耗时: ${Date.now() - startTime}ms`);

    // 2. 加钱 (必须事务内) - 关闭 runValidators 提速
    if (payout > 0) {
      finalBalance = balanceAfterBet + payout;
      await User.updateOne({ _id: userId }, { $inc: { balance: payout } }, { session, runValidators: false });
    }

    // ★★★ 核心改造：立刻提交事务，释放数据库锁！ ★★★
    await session.commitTransaction();
    session.endSession();
    appLogger.info(`[消消乐-耗时3] 事务提交总耗时: ${Date.now() - startTime}ms`);

    // 3. 立刻返回给前端！用户不用再等后面的写入了！
    res.json({ totalScore, fee, payout, netProfit, balance: finalBalance, ticketPrice, frames });

    // ★★★ 核心改造：非核心数据异步写入，不阻塞响应 ★★★
    try {
