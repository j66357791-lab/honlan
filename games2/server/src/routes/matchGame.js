    ...
    for (let c = 0; c < BOARD_SIZE; c++) {
      ...
      for (let r = BOARD_SIZE - 1; r >= 0; r--) {
        if (!eliminated.has(`${r}-${c}`)) {
          ...
          if (r !== writeRow) {
            moves.push({ uid: block.uid, type: block.type, fromRow: r, toRow: writeRow, col: c });
          }
          writeRow--;
        }
      }
...
const stats = { total: 0, wins: 0, losses: 0, totalWagered: 0, totalPayout: 0, totalFee: 0 };
...
router.post('/bet', authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate(
      ...
      { $inc: { balance: -ticketPrice } },
      { new: true }
    );
    if (!user) {
      return res.status(400).json({ error: '积分不足或账号异常' });
    }
    const balanceAfterBet = user.balance;

    // 2. 纯内存计算游戏
    const initSeed = crypto.randomBytes(8).toString('hex');
    const result = simulateGame(ticketPrice, initSeed);
    const { totalScore, waves, frames, newBlockSeeds } = result;
    ...
    // 更新内存统计
    stats.total++;
    if (netProfit >= 0) stats.wins++; else stats.losses++;
    stats.totalWagered += ticketPrice;
    stats.totalPayout += payout;
    stats.totalFee += fee;
    console.log(`[消消乐] 门票=${ticketPrice} 总分=${totalScore} 净赚=${netProfit} 波数=${waves} 种子=${initSeed}`);
    ...
    Promise.all([
      Transaction.create([
        ...
        ...(payout > 0 ? [{ userId, type: 'bet_win', amount: payout, balanceAfter: finalBalance, remark: `消消乐派彩 +${payout}${fee > 0 ? ` (手续费-${fee})` : ''}`, gameType: 'match' }] : [])
      ]),
      MatchBet.create([{
        userId,
      ...
      console.error('[消消乐] 后台记账失败，请注意排查:', err.message);
    });

  } catch (err) {
    console.error('[消消乐] ❌ 异常:', err.message);
    if (!res.headersSent) {
      res.status(500).json({ error: '游戏失败，请稍后重试' });
    }
  }
});
