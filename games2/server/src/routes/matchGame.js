...
router.post('/bet', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // ★★★ 核心改造：池子空了直接快速失败，绝不同步生成卡死主线程 ★★★
    if (!result) {
      await session.abortTransaction();
      session.endSession();
      // 返回 503 状态码，前端可根据此提示做友好展示或自动重试
    ...
    await MatchBet.create([{ userId, roundId: crypto.randomUUID(), ticketPrice, totalScore, netProfit, waves, initSeed, newBlockSeeds }], { session, ordered: true });
