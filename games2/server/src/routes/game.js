  ...
  try {
    ...
    if (decision.constraints?.forceOpposite) {
      ...
      if (decision.modifier && Object.keys(decision.modifier).length > 0) {
        const allSame = finalProb.red !== undefined &&
          decision.modifier.red !== undefined &&
          decision.modifier.blue !== undefined &&
          decision.modifier.red === decision.modifier.blue;
      ...
      const netLossMap = {
        red: calcSystemLoss('red'),
        blue: calcSystemLoss('blue'),
        draw: calcSystemLoss('draw')
      };
      ...
      if (safeProbMap) {
        for (const [key, prob] of Object.entries(safeProbMap)) {
          cumulative += prob;
          if (rand < cumulative) { result = key; break; }
        }
      } else {
    ...
    await Transaction.create([{
      ...
      remark: `下注 红${redAmount}/平${drawAmount}/蓝${blueAmount} 共${betAmount}积分`,
      roundId
    }], { session });
    ...
    if (totalPayout > 0) {
      ...
      await Transaction.create([{
        ...
        remark: `赢得${winChoices}赔付 ${totalPayout}积分`,
        roundId
      }], { session });
    }
    ...
    // 风控结算（异步，不影响主流程）
    riskEngine.settle(userId, 'giant_racing', betAmount, totalPayout, {
      choices: actualChoices,
    }).catch(() => {});
