...
router.post('/bet', authMiddleware, async (req, res) => {
  try {
    ...
    if (decision.constraints.forceOpposite) {
      console.log(`[巨人暗箱] 🕵️‍♂️ 触发强制反向开奖！玩家死磕: ${choice}`);
      if (choice === 'red') result = 'blue';
      else if (choice === 'blue') result = 'red';
      else result = crypto.randomInt(2) === 0 ? 'red' : 'blue';
      console.log(`[巨人暗箱] 🎯 强制开出: ${result}`);
    } else {
      ...
      if (decision.modifier) {
        ...
        if (allFactor < 1.0) {
          // 风控要求压低概率：只压玩家押的，提权没押的
          finalProb[choice] *= allFactor;
          const otherChoices = ['red', 'blue', 'draw'].filter(c => c !== choice);
          const boostFactor = 1.0 + (1.0 - allFactor) * 0.8;
          otherChoices.forEach(c => {
            finalProb[c] *= boostFactor;
          });
        } else if (allFactor > 1.0) {
          // 风控要求放水：只提权玩家押的，压没押的
          finalProb[choice] *= allFactor;
          const otherChoices = ['red', 'blue', 'draw'].filter(c => c !== choice);
          const suppressFactor = 1.0 / allFactor;
          otherChoices.forEach(c => {
            finalProb[c] *= suppressFactor;
          });
        } else {
