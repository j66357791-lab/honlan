...
router.get('/activation-codes', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const codes = await ActivationCode.find(query)
      ...
      .populate('usedBy', 'uid phone nickname');
...
// 获取当前维护状态
router.get('/maintenance-status', authMiddleware, adminMiddleware, async (req, res) => {
  res.json({ isMaintenance: global.isMaintenanceMode || false });
});

// 一键切换维护模式
router.post('/toggle-maintenance', authMiddleware, adminMiddleware, async (req, res) => {
  // 取反：开着就关，关着就开
  global.isMaintenanceMode = !global.isMaintenanceMode;

  res.json({ 
    message: global.isMaintenanceMode ? '已开启全站维护模式' : '已关闭维护模式，恢复正常',
    isMaintenance: global.isMaintenanceMode
  });
});
