...
router.get('/stats', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      totalUsers,
      totalPlayerBalance,
      totalAdminAdd: pool?.totalAdminAdd || 0,     // 缓存字段
      totalAdminSub: pool?.totalAdminSub || 0,     // 缓存字段
...
router.post('/toggle-internal', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: `用户 ${user.phone} 已${newInternal ? '设为内部账号' : '恢复为普通账号'}`, isInternal: newInternal });
  } catch (err) {
    res.status(500).json({ error: '操作失败' });
  }
});
...
router.get('/bets', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const giantBets = await Bet.find()
      ...
      .sort({ createdAt: -1 })
      .limit(limit)
      .select('-bets -roundId -__v'); // 排除不需要的字段
...
router.put('/user-profile', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    await user.save();
    res.json({ message: '用户资料修改成功', user: { id: user._id, uid: user.uid, nickname: user.nickname, avatar: user.avatar, phone: user.phone.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2') } });
  } catch (err) {
