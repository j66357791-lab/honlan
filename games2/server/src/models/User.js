...
const userSchema = new mongoose.Schema(
  {
    phone: {
      type: String, required: true, unique: true, trim: true, match: /^1[3-9]\d{9}$/,
    },
    uid: {
      type: String, unique: true, sparse: true,
    },
    nickname: {
      type: String, default: '', trim: true, maxlength: 12,
    },
    avatar: {
    ...
    vipLastClaimDate: { type: String, default: '' },
    
    // ★ 新增：SVIP独立字段
    svipExpireAt: { type: Date, default: null },
    svipLastClaimDate: { type: String, default: '' },

    // 废弃旧字段(保留防报错，不再使用)
    membershipType: { type: String, enum: ['none', 'normal', 'super'], default: 'none' },
    membershipExpireAt: { type: Date, default: null },
    membershipLastClaimDate: { type: String, default: '' },

    claimedRedPackets: { type: [Number], default: [] },
