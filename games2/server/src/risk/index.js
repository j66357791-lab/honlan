...
export async function initRiskModule() {
  ...
  const usersWithoutProfile = await User.find({
    _id: { $nin: (await RiskProfile.distinct('userId')) }
  });
  ...
  console.log(`🎰 [风控] 模块初始化完成`);
  console.log(`   奖池净收益: ${pool.netProfit} 积分 (收益率: ${(pool.profitRate * 100).toFixed(2)}%)`);
  console.log(`   用户画像: ${await RiskProfile.countDocuments()} 条`);
  console.log(`   活跃规则: ${RULES.length} 条`); // ← 修复：直接使用顶部导入的 RULES
}
