...
class RiskEngine {
  ...
  async evaluate({ type, userId, gameId, payload }) {
    try {
      ...
      this._logDecision(finalDecision, userId, payload.gameId || 'unknown');

      return { ...finalDecision, context };
    } catch (err) {
      appLogger.error('[风控引擎] 评估异常:', err);
      return {
        action: 'ALLOW',
        modifier: {},
        constraints: {},
  ...
  /** ★★★ 内部方法：风控日志分级打印 ★★★ */
  _logDecision(decision, userId, gameId) { 
    const shortUserId = userId.toString().slice(-4);
    const logPrefix = `[风控][用户:${shortUserId}][游戏:${gameId}]`;
    
    // 1. 强压制 / 黑名单 / 阻断 (Warn级别，控制台亮黄字)
    if (decision.action === 'BLOCK' || decision.gameImpact >= 0.8) {
      appLogger.warn(`${logPrefix} 压制 | ${decision.reason}`);
      return;
    }
    
    // 2. 放水阶段 (Info级别，常规蓝色信息)
    if (decision.eatPhase === 'FEED') {
      appLogger.info(`${logPrefix} 放水 | ${decision.reason}`);
      return;
    }
    
    // 3. 普通调整 (中度压制或安抚，Info级别)
    if (decision.modifier && Object.keys(decision.modifier).length > 0) {
      appLogger.info(`${logPrefix} 调整 | ${decision.reason}`);
      return;
    }
    
    // 4. 纯放行 (debug级别，只写文件不刷控制台)
    appLogger.debug(`${logPrefix} 放行 | ${decision.reason}`);
  }
