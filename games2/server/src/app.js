...
if (process.env.NODE_ENV === 'production') { const noop = () => {}; console.log = noop; console.info = noop; console.warn = noop; }
...
const corsOptions = { origin: function (origin, callback) { if (!origin || whitelist.includes(origin)) { callback(null, true); } else { console.warn(`[安全拦截] 不允许的跨域请求来源: ${origin}`); callback(new Error('Not allowed by CORS')); } }, credentials: true };
const apiLimiter = rateLimit({ windowMs: 1 * 60 * 1000, max: 60, message: { error: '操作过于频繁，请稍后再试' }, standardHeaders: true, legacyHeaders: false, });

app.use(helmet()); app.use(cors(corsOptions)); app.use(express.json()); app.use(morgan('combined'));
...
app.use('/api', authRoutes); app.use('/api', riskDailyReset); app.use('/api', gameRoutes);
app.use('/api/admin', adminRoutes); app.use('/api/pointing', pointingGameRoutes);
app.use('/api/chat', chatRoutes); app.use('/api/match', matchGameRoutes);
...
async function start() {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(' 特殊(10倍)：关羽'); console.log(' 特殊(20倍)：张飞');
      console.log(' 特殊(30倍)：梁红玉'); console.log(' 特殊(40倍)：穆桂英');
      console.log(' 抽水：固定5%（仅对净盈利部分收取）');
      console.log('------------------------------------------');
      console.log('🧩 [消消乐] 参数配置:'); // ★ 新增日志区
