/**
 * 客服系统 - WebSocket 网关 v3.1
 * [v3.1] 修复：解决用户重连时旧连接断开误删新连接，导致收不到消息的严重Bug
...
// ==================== 自动回复规则 ====================
const AUTO_RULES = [
  { keywords: ['充值', '充钱', '怎么充', '如何充值', '充值积分', '上分', '充值多少'], reply: '💰 充值请联系人工客服处理，回复"转人工"即可为您服务。' },
  { keywords: ['规则', '怎么玩', '玩法', '游戏规则', '怎么下注', '赔率'], reply: '🎮 巨人赛跑：红/蓝1.9倍，平局9倍\n⚔️ 点兵点将：男/女将1.9倍，武力越低赔率越高\n需要详细说明请回复"转人工"。' },
  { keywords: ['提现', '提钱', '兑换', '怎么提', '下分', '兑换积分'], reply: '💸 提现请联系人工客服处理，回复"转人工"即可。' },
  { keywords: ['账号', '异常', '登录不了', '封号', '解封', '冻结', '被禁'], reply: '🔒 账号问题需人工核实，回复"转人工"联系客服。' },
  { keywords: ['人工', '转人工', '人工客服', '真人', '接线员', '客服'], reply: null },
];
...
class ChatGateway {
  ...
  async handleConnection(ws, req) {
    ...
    // ★ 核心修复1：建立新连接时，主动关闭该用户的旧连接，防止僵尸连接
    const oldWs = pool.get(ws.userId);
    if (oldWs && oldWs !== ws && oldWs.readyState === WebSocket.OPEN) {
      try { oldWs.close(4004, '账号在其他地方登录或重连'); } catch {}
    }
  ...
  handleDisconnect(ws) {
    ...
    // ★ 核心修复2：只有当 Map 里存的连接就是当前断开的连接时，才删除，防止误删新连接
    if (pool.get(ws.userId) === ws) {
  ...
  async pushSessionList(ws) {
    try {
      const sessions = await ChatSession.find({})
        .sort({ updatedAt: -1 })
        .limit(100);
      const list = sessions.map(s => this._formatSession(s));
      this.sendToClient(this.adminClients, ws.userId, { type: 'sessions_list', sessions: list });
    } catch (err) {
  ...
  async pushUserSession(ws) {
    try {
      const session = await ChatSession.findOne({ userId: ws.userId, status: { $in: ['waiting', 'active'] } });
      if (session) {
        this.sendToClient(this.userClients, ws.userId, {
          type: 'user_session',
          session: { id: String(session._id), status: session.status, adminName: session.adminName }
        });
      }
    } catch {}
  }
  ...
  async broadcastSessionUpdate() {
    ...
    for (const [, ws] of this.adminClients) {
      if (ws.readyState === WebSocket.OPEN) {
        tasks.push(this.pushSessionList(ws));
      }
    }
  ...
  // ==================== ★ 广播增量更新给所有管理员（性能优化核心） ====================
  broadcastSessionIncrement(session) {
    const data = { type: 'session_update', session: this._formatSession(session) };
  ...
  async handleChatSend(ws, msg) {
    ...
    if (ws.userRole === 'admin') {
      ...
      session = await ChatSession.findOne({ userId: ws.userId, status: { $in: ['waiting', 'active'] } });
      if (!session) {
        session = await ChatSession.create({ userId: ws.userId, userName: ws.userName, status: 'waiting' });
        isNewSession = true;
      }
    }
    ...
    // ★ 性能优化：新会话全量推送，旧会话增量推送
    if (isNewSession) {
      this.broadcastToAdmins({ type: 'session_new', session: this._formatSession(session) });
      this.broadcastSessionUpdate(); // 新会话产生，全量同步一次列表确保一致
    } else {
      // 普通消息仅推送被更新的会话
      this.broadcastSessionIncrement(session);
    }
    ...
    setInterval(async () => {
      try {
        ...
        if (result.deletedCount > 0) {
          ...
          this.broadcastSessionUpdate(); // 清理后全量推送一次列表
        }
      } catch (err) {
