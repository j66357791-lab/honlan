/** * 客服系统 - WebSocket 网关 v3 * [v3] 纯中转架构：不存消息到DB，仅用WebSocket中转 * - 消息只保存在用户/客服的localStorage * - ChatSession仅保留会话元数据（状态、未读数、最后消息预览） * - 会话列表通过WebSocket推送，不再轮询 * - 12小时自动清理过期会话 */ 
...
class ChatGateway { 
  ...
  async handleSessionAccept(ws, msg) { 
    ...
    // 状态发生重大变化，推送增量更新即可（前端会自动更新列表和当前会话状态）
    this.broadcastSessionIncrement(session); 
